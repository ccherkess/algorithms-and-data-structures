...
public class SortingStationAlgorithm {
	...
	public static Queue<Object> parsingPostfixForm(String input) throws ErrorInExpressionException, ThereIsNoSuchOperatorException {
		...
		Stack<Operator> stack = new Stack<>(new DoubleLinkedList<Operator>()); //стэк для операторов
		
		Queue<Object> output = new Queue<>(); //выходная очередь
		...
		for (int i = 0; i < input.length(); i++) {
			...
			if (isOperand(input.charAt(i))) { 
				...
				//если считываем отрицательно число
				if (stack.size() == 1 && output.isEmpty() && stack.top() == Operator.MINUS) {
				...
				while (!stack.isEmpty() && stack.top() != Operator.OPENING_PARENTHESIS) {
				...
				if (stack.isEmpty()) throw new ErrorInExpressionException("отсутствует открывающая скобка");
				
				stack.pop(); //выводим ( из стэка
				
				//если на вершине стэка лежит функция (приоритет функций равен 4) - выводим её в выходную очередь
				if (!stack.isEmpty() && stack.top().priority == 4) output.push(stack.pop());
				...
				if (operator.priority != 4) {  //если оператор не функция (приоритет функций равен 4)
					 ...
					 */
					while (!stack.isEmpty() 
							&& stack.top().priority >= operator.priority
							&& stack.top() != Operator.OPENING_PARENTHESIS) {
		...
		while (!stack.isEmpty()) {
			...
			if (stack.top() == Operator.OPENING_PARENTHESIS) throw new ErrorInExpressionException("отсутствует открывающая скобка");
	...
	public static double calculateExpression(Queue<Object> expression) throws NotEnoughArgumentsException {
		...
		while (!expression.isEmpty()) {
			...
			//если встретился операнд, то помещаем в его в стэк
			if (expression.top().getClass() == Double.class) stack.push( (Double) expression.pop()); 
			else { //если встретился оператор
				
				Operator operator = (Operator) expression.pop(); //извлекаем оператор из стэка
